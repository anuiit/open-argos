---
name: mos-sota
description: Run mosaic SOTA Explorer for source-backed deep web/academic research over a domain or question. Use `mosaic sota` / `mosaic @sota-explorer` with bounded sources, evidence artifacts, two synthesis models, and a final reviewer.
---
...
```bash
mosaic @sota-normal "<domain or question>" --json
mosaic @sota-deep "<domain or question>" --json
```

Use normal for a daily bounded pass and deep for a wider source/query budget. Plain `mosaic sota` defaults to normal.
