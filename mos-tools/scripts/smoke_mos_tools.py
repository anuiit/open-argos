#!/usr/bin/env python3
"""Non-destructive Mos-Tools/mosaic smoke test.

Default mode avoids expensive live multi-mosaic presets. Use --live for a tiny
...
def main() -> int:
    ...
    parser.add_argument("--live", action="store_true", help="run small paid/live mosaic checks")
    parser.add_argument("--vision", action="store_true", help="include agy vision live check; implies --live")
    parser.add_argument("--sota", action="store_true", help="include retrieval-only mosaic sota smoke over public sources")
    parser.add_argument("--adversarial", action="store_true", help="run two break-oriented smoke checks per Mos-Tools/mosaic feature")
    ...
    parser.add_argument("--mosaic-py", help="mosaic.py path forwarded to adversarial smoke")
    parser.add_argument("--artifact-root", help="mosaic artifact root for gate/SOTA smoke artifacts")
    parser.add_argument("--no-gate", action="store_true", help="skip writing the mos-tools-smoke gate artifact")
    args = parser.parse_args()
    if args.vision:
        args.live = True

    doctor = run(["mosaic", "doctor"], check=False)
    doctor_data = parse_json_output(doctor, "mosaic doctor")
    core_ready = bool(doctor_data.get("readiness", {}).get("core_text_mosaics"))
    ...
    if doctor.returncode != 0 and not ((args.vision or args.live) and agy_ready):
        raise SystemExit(f"mosaic doctor returned {doctor.returncode}; core text mosaics are not ready")
    ping = run(["mosaic", "ping", "--json"])
    data = parse_json_output(ping, "mosaic ping --json")
    if data.get("status") != "ok":
        raise SystemExit("mosaic ping returned non-ok status")
    run(["mosaic", "providers", "--json"])
    if not args.no_gate:
        gate_cmd = ["mosaic", "gate", "set", "mos-tools-smoke", "pass", "--evidence", "smoke script static checks passed"]
    ...
    if args.live and not args.vision:
        if not core_ready:
            raise SystemExit("--live text smoke requires core text mosaics from mosaic doctor")
        run(["mosaic", "@review", "Smoke test only. Reply PASS.", "--mosaic", "minimax", "--single-ok", "--json"], timeout=180)
    if args.vision:
        if not agy_ready:
            raise SystemExit("--vision smoke requires optional_agy_vision_cli from mosaic doctor")
        with tempfile.TemporaryDirectory() as td:
            image = Path(td) / "mos-tools-smoke.png"
            write_png(image)
            run(["mosaic", "@vision", "Identify the two main colors only.", "--image", str(image), "--json"], timeout=180)
    if args.adversarial:
        adversarial_cmd = [sys.executable, str(Path(__file__).with_name("adversarial_smoke_mos_tools.py"))]
        if args.mosaic_py:
            adversarial_cmd.extend(["--mosaic-py", args.mosaic_py])
    ...
    if args.sota:
        ...
        if proc.returncode not in {0, 2}:
            raise SystemExit(f"mosaic sota smoke crashed ({proc.returncode})")
        payload = parse_json_output(proc, "mosaic sota smoke")
        artifact = Path(payload["artifact_dir"])
        if payload.get("mode") != "sota" or not (artifact / "report.md").exists():
            raise SystemExit("mosaic sota smoke did not produce expected artifacts")
    print("Mos-Tools smoke: PASS")
    return 0
