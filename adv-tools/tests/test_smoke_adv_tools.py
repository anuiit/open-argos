...
SMOKE_PATH = Path(__file__).resolve().parents[1] / "scripts" / "smoke_adv_tools.py"
ADVERSARIAL_SMOKE_PATH = Path(__file__).resolve().parents[1] / "scripts" / "adversarial_smoke_adv_tools.py"
spec = importlib.util.spec_from_file_location("smoke_adv_tools_under_test", SMOKE_PATH)
assert spec and spec.loader
smoke = importlib.util.module_from_spec(spec)
sys.modules[spec.name] = smoke
spec.loader.exec_module(smoke)
adv_spec = importlib.util.spec_from_file_location("adversarial_smoke_adv_tools_under_test", ADVERSARIAL_SMOKE_PATH)
...
class SmokeAdvToolsTests(unittest.TestCase):
    def run_main(self, argv: list[str], fake_run):
        out = io.StringIO()
        err = io.StringIO()
        with mock.patch.object(sys, "argv", ["smoke_adv_tools.py", *argv]), \
    ...
    def test_default_smoke_runs_static_checks_and_gate_only(self) -> None:
        ...
        def fake_run(cmd, **kwargs):
            calls.append(cmd)
            if cmd == ["advisor", "doctor"]:
                return completed(cmd, json.dumps({"readiness": {"core_text_advisors": True}}))
            if cmd == ["advisor", "ping", "--json"]:
                return completed(cmd, json.dumps({"status": "ok"}))
            if cmd == ["advisor", "providers", "--json"]:
                return completed(cmd, json.dumps({"status": "ok"}))
            if cmd[:3] == ["advisor", "gate", "set"]:
                return completed(cmd, "adv-tools-smoke\tpass\t/tmp/gate.json")
            self.fail(f"unexpected command: {cmd}")

        rc, out, _err = self.run_main([], fake_run)
        self.assertEqual(rc, 0)
        self.assertIn("Adv-Tools smoke: PASS", out)
        self.assertEqual(calls, [
            ["advisor", "doctor"],
            ["advisor", "ping", "--json"],
            ["advisor", "providers", "--json"],
            ["advisor", "gate", "set", "adv-tools-smoke", "pass", "--evidence", "smoke script static checks passed"],
        ])
    ...
    def test_no_gate_skips_gate_write(self) -> None:
        ...
        self.assertEqual(rc, 0)
        self.assertIn("Adv-Tools smoke: PASS", out)
        self.assertFalse(any(cmd[:3] == ["advisor", "gate", "set"] for cmd in calls))
    ...
    def test_malformed_json_fails_with_context_not_traceback(self) -> None:
        ...
        with mock.patch.object(sys, "argv", ["smoke_adv_tools.py"]), \
            mock.patch.object(smoke, "run", side_effect=fake_run):
            with self.assertRaises(SystemExit) as raised:
                smoke.main()
        self.assertIn("advisor doctor did not return valid JSON", str(raised.exception))
    ...
    def test_vision_implies_live_but_does_not_run_text_live_branch(self) -> None:
        ...
        def fake_run(cmd, **kwargs):
            calls.append(cmd)
            if cmd == ["advisor", "doctor"]:
                return completed(cmd, json.dumps({"readiness": {"core_text_advisors": True, "optional_agy_vision_cli": True}}))
            if cmd == ["advisor", "ping", "--json"]:
                return completed(cmd, json.dumps({"status": "ok"}))
            if cmd == ["advisor", "providers", "--json"]:
                return completed(cmd, json.dumps({"status": "ok"}))
            ...
            if cmd[:3] == ["advisor", "@vision", "Identify the two main colors only."]:
                return completed(cmd, json.dumps({"status": "ok"}))
            self.fail(f"unexpected command: {cmd}")
        ...
        self.assertEqual(rc, 0)
        self.assertTrue(any(cmd[:2] == ["advisor", "@vision"] for cmd in calls))
        self.assertFalse(any(cmd[:2] == ["advisor", "@review"] for cmd in calls))
    ...
    def test_adversarial_flag_runs_adversarial_suite_after_static_checks(self) -> None:
        ...
        def fake_run(cmd, **kwargs):
            ...
            if cmd[:2] == [sys.executable, str(ADVERSARIAL_SMOKE_PATH)]:
                return completed(cmd, "Adv-Tools adversarial smoke: PASS")
            self.fail(f"unexpected command: {cmd}")
    ...
    def test_adversarial_advisor_py_flag_is_forwarded(self) -> None:
        calls: list[list[str]] = []
        ...
        rc, _out, _err = self.run_main(["--adversarial", "--advisor-py", "/tmp/custom-advisor.py"], fake_run)
        self.assertEqual(rc, 0)
        adversarial_calls = [cmd for cmd in calls if cmd[:2] == [sys.executable, str(ADVERSARIAL_SMOKE_PATH)]]
        self.assertEqual(adversarial_calls, [[sys.executable, str(ADVERSARIAL_SMOKE_PATH), "--advisor-py", "/tmp/custom-advisor.py"]])


class AdversarialSmokeTests(unittest.TestCase):
    def test_feature_requires_exactly_two_checks(self) -> None:
        suite = adversarial.Suite(advisor=object())
