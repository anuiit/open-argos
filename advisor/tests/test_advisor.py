...
ADVISOR_PATH = Path(__file__).resolve().parents[1] / "advisor.py"
spec = importlib.util.spec_from_file_location("advisor_under_test", ADVISOR_PATH)
...
class ConfigValidationTests(unittest.TestCase):
    def test_version_tracks_sota_release(self) -> None:
        self.assertTrue(advisor.VERSION.startswith("0.6."))

    def test_load_env_file_does_not_override_existing_values(self) -> None:
        with tempfile.TemporaryDirectory() as td, mock.patch.dict(os.environ, {"ADVISOR_TEST_ENV": "original"}, clear=False):
            env_path = Path(td) / ".env"
            env_path.write_text("ADVISOR_TEST_ENV=changed\nADVISOR_TEST_NEW=loaded\n# ignored\nBAD-KEY=nope\n", encoding="utf-8")
            try:
                os.environ.pop("ADVISOR_TEST_NEW", None)
                advisor.load_env_file(env_path)
                self.assertEqual(os.environ["ADVISOR_TEST_ENV"], "original")
                self.assertEqual(os.environ["ADVISOR_TEST_NEW"], "loaded")
                self.assertNotIn("BAD-KEY", os.environ)
            finally:
                os.environ.pop("ADVISOR_TEST_NEW", None)

    def test_default_sonnet_points_to_claude_sonnet_5(self) -> None:
        self.assertEqual(advisor.DEFAULT_CONFIG["models"]["sonnet"][0]["model"], "claude-sonnet-5")

    def test_validate_config_rejects_codex_model_or_kind(self) -> None:
        cfg = advisor.deep_merge(advisor.DEFAULT_CONFIG, {
    ...
    def test_preset_cross_references_are_validated(self) -> None:
        cfg = advisor.deep_merge(advisor.DEFAULT_CONFIG, {
            "presets": {"@bad": {"mode": "review", "advisors": ["missing"]}},
        })
        with self.assertRaises(SystemExit):
            advisor.validate_config(cfg)
    ...
    def test_validate_config_rejects_unknown_sota_source(self) -> None:
        cfg = advisor.deep_merge(advisor.DEFAULT_CONFIG, {"sota": {"sources": ["arxiv", "nope"]}})
        with self.assertRaises(SystemExit):
            advisor.validate_config(cfg)

    def test_validate_config_does_not_brick_core_for_sota_model_refs(self) -> None:
        cfg = advisor.deep_merge(advisor.DEFAULT_CONFIG, {"sota": {"reviewer": "missing", "synthesizers": ["also_missing"]}})
        advisor.validate_config(cfg)


    def test_agy_default_concurrency_is_conservative(self) -> None:
        self.assertEqual(advisor.concurrency_limit(advisor.DEFAULT_CONFIG, "agy"), 2)

    def test_default_config_has_only_agy_vision_provider(self) -> None:
        self.assertNotIn("ge" + "mini_image", advisor.DEFAULT_CONFIG["models"])
        self.assertNotIn("ge" + "mini", advisor.DEFAULT_CONFIG["concurrency"])
        self.assertEqual(advisor.DEFAULT_CONFIG["modes"]["vision"], ["agy_image"])
    ...
    def test_cli_main_maps_invalid_config_to_exit_error(self) -> None:
        with tempfile.TemporaryDirectory() as td, contextlib.redirect_stderr(io.StringIO()) as err:
            ...
            rc = advisor.cli_main(["--config", str(bad), "doctor"])
            self.assertEqual(rc, advisor.EXIT_ERROR)
    ...
    def test_validate_config_rejects_malformed_candidates(self) -> None:
        ...
        for override in bad_configs:
            with self.subTest(override=override):
                ...
                cfg = advisor.deep_merge(advisor.DEFAULT_CONFIG, test_override)
                with self.assertRaises(SystemExit):
                    advisor.validate_config(cfg)
    ...
    def test_validate_config_rejects_invalid_runtime_numbers(self) -> None:
        ...
        for override in bad_overrides:
            with self.subTest(override=override):
                cfg = advisor.deep_merge(advisor.DEFAULT_CONFIG, override)
                with self.assertRaises(SystemExit):
                    advisor.validate_config(cfg)
...
class PromptAndRoutingTests(unittest.TestCase):
    def test_build_prompt_embeds_file_once_with_cap(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            cfg = advisor.deep_merge(advisor.DEFAULT_CONFIG, {"limits": {"file_chars": 3}})
            prompt = advisor.build_prompt("review", "please review", [path], cfg)
    ...
    def test_build_prompt_applies_total_prompt_cap(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            cfg = advisor.deep_merge(advisor.DEFAULT_CONFIG, {"limits": {"file_chars": 100, "total_prompt_chars": 120}})
            prompt = advisor.build_prompt("review", "please review", [p1, p2], cfg)
    ...
    def test_build_prompt_total_cap_is_strict_for_tiny_limits(self) -> None:
        cfg = advisor.deep_merge(advisor.DEFAULT_CONFIG, {"limits": {"total_prompt_chars": 10}})
        prompt = advisor.build_prompt("review", "x" * 100, [], cfg)
        self.assertLessEqual(len(prompt), 10)

    def test_build_prompt_lists_image_paths_and_mime(self) -> None:
        image = Path(tempfile.gettempdir()) / "advisor-test-image.png"
        prompt = advisor.build_prompt("vision", "describe", [], advisor.DEFAULT_CONFIG, [image])
    ...
    def test_build_prompt_marks_embedded_files_as_untrusted(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            prompt = advisor.build_prompt("review", "please review", [path], advisor.DEFAULT_CONFIG)
    ...
    def test_build_prompt_includes_structured_output_contract(self) -> None:
        prompt = advisor.build_prompt("review", "please review", [], advisor.DEFAULT_CONFIG)
    ...
    def test_apply_persona_respects_total_prompt_cap(self) -> None:
        cfg = advisor.deep_merge(advisor.DEFAULT_CONFIG, {"limits": {"total_prompt_chars": 120}})
        prompt, meta = advisor.apply_persona("sonnet", "x" * 500, cfg)
        self.assertIsNotNone(meta)
        self.assertLessEqual(len(prompt), 120)
        self.assertIn("prompt truncated to 120 chars", prompt)

    def test_truncate_prompt_total_disabled_with_non_positive_limit(self) -> None:
        cfg = advisor.deep_merge(advisor.DEFAULT_CONFIG, {"limits": {"total_prompt_chars": 0}})
        self.assertEqual(advisor.truncate_prompt_total("abc", cfg), "abc")

    def test_resolve_preset_uses_preset_advisors_unless_explicit(self) -> None:
        mode, advisors, preset = advisor.resolve_mode_and_advisors("@review", None, advisor.DEFAULT_CONFIG)
        self.assertEqual(mode, "review")
        self.assertEqual(advisors, ["sonnet", "kimi", "minimax"])
        self.assertEqual(preset, "@review")

        mode, advisors, preset = advisor.resolve_mode_and_advisors("@review", ["sonnet"], advisor.DEFAULT_CONFIG)
        self.assertEqual(mode, "review")
        self.assertEqual(advisors, ["sonnet"])
        self.assertEqual(preset, "@review")

    def test_resolve_raw_mode_uses_configured_default_advisors(self) -> None:
        mode, advisors, preset = advisor.resolve_mode_and_advisors("review", None, advisor.DEFAULT_CONFIG)
        self.assertEqual(mode, "review")
        self.assertEqual(advisors, ["sonnet", "kimi", "minimax"])
        self.assertIsNone(preset)

    def test_resolve_raw_mode_keeps_explicit_advisors(self) -> None:
        mode, advisors, preset = advisor.resolve_mode_and_advisors("review", ["sonnet"], advisor.DEFAULT_CONFIG)
        self.assertEqual(mode, "review")
        self.assertEqual(advisors, ["sonnet"])
        self.assertIsNone(preset)
...
class ProviderParsingTests(unittest.TestCase):
    def test_parse_opencode_jsonl_text_and_usage(self) -> None:
        ...
        content, meta = advisor.parse_opencode(stdout)
    ...
    def test_parse_claude_json(self) -> None:
        content, meta = advisor.parse_claude(json.dumps({
            "result": " done ",
            "session_id": "claude_1",
            "total_cost_usd": 0.34,
    ...
    def test_parse_claude_ignores_surrounding_cli_noise(self) -> None:
        content, meta = advisor.parse_claude('warning before\n' + json.dumps({
            "result": " ok ",
            "session_id": "claude_2",
    ...
    def test_parse_agy_text(self) -> None:
        content, meta = advisor.parse_agy("visual answer\n")
    ...
    def test_validated_image_paths_rejects_non_image(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            with self.assertRaises(SystemExit):
                advisor.validated_image_paths([str(path)])
    ...
    def test_validated_image_paths_accepts_registered_modern_formats(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual([path.suffix for path in advisor.validated_image_paths(inputs)], [".webp", ".heic", ".heif"])
...
class ArtifactTests(unittest.TestCase):
    def test_make_artifact_dir_avoids_same_second_collision_and_updates_latest(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td, mock.patch.object(advisor.dt, "datetime", FixedDateTime):
            root = Path(td)
            first = advisor.make_artifact_dir(root, "review")
            second = advisor.make_artifact_dir(root, "review")
...
class SubprocessTimeoutTests(unittest.TestCase):
    ...
    def test_run_candidate_rejects_minimax_overrides_before_subprocess(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td:
            runner = advisor.Runner(advisor.DEFAULT_CONFIG, Path(td))
    ...
    def test_run_candidate_builds_agy_command_with_unique_image_dirs(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td, mock.patch.object(advisor, "run_subprocess", side_effect=fake_run_subprocess):
            root = Path(td)
            img1 = root / "one.png"
            img2 = root / "two.png"
            img1.write_bytes(b"x")
            img2.write_bytes(b"x")
            runner = advisor.Runner(advisor.DEFAULT_CONFIG, root)
            result = asyncio.run(runner.run_candidate(
                "agy_image",
        ...
        self.assertEqual(call["timeout"], advisor.DEFAULT_CONFIG["timeouts"]["agy"] + 5)
        cmd = call["cmd"]
    ...
    def test_stage_vision_images_is_idempotent_for_already_staged_inputs(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            first = advisor.stage_vision_images(root, [original])
            second = advisor.stage_vision_images(root, first)
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            try:
                with mock.patch.dict(os.environ, {"PATH": str(root) + os.pathsep + old_path}):
                    rc, out, err, _dur = asyncio.run(advisor.run_subprocess(["opencode", str(pidfile)], timeout=1, cwd=root))
...
class CrossProcessConcurrencyTests(unittest.TestCase):
    def test_cross_process_slot_contention_times_out_and_releases(self) -> None:
        ...
        async def scenario() -> None:
            with tempfile.TemporaryDirectory() as td, mock.patch.object(advisor, "DEFAULT_LOCK_ROOT", Path(td)):
                first = advisor.CrossProcessSlots(cfg, [("test_provider", 1)])
                async with first:
                    with self.assertRaises(TimeoutError):
                        async with advisor.CrossProcessSlots(cfg, [("test_provider", 1)]):
                            pass
                async with advisor.CrossProcessSlots(cfg, [("test_provider", 1)]):
                    pass
...
class RunListingTests(unittest.TestCase):
    def test_list_runs_reports_one_shot_artifacts_without_sessions(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            (run / "meta.json").write_text(json.dumps({
                "mode": "review",
                "results": [
                    {"advisor": "sonnet", "status": "ok"},
                    {"advisor": "kimi", "status": "error"},
                ],
            }), encoding="utf-8")
            ...
            with contextlib.redirect_stdout(out):
                rc = advisor.list_runs(root, as_json=True)
    ...
    def test_list_sessions_still_reports_only_multiturn_sessions(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            out = io.StringIO()
            with contextlib.redirect_stdout(out):
                rc = advisor.list_sessions(root, as_json=True)
            rows = json.loads(out.getvalue())
...
class SafetyAndSessionTests(unittest.TestCase):
    def test_subprocess_allowlist_blocks_codex_and_ollama(self) -> None:
        advisor.assert_allowed_subprocess(["opencode", "run"])
        advisor.assert_allowed_subprocess(["claude", "-p"])
        advisor.assert_allowed_subprocess(["agy", "--print", "x"])
        with self.assertRaises(RuntimeError):
            advisor.assert_allowed_subprocess(["ge" + "mini", "--prompt", "x"])
        with self.assertRaises(RuntimeError):
            advisor.assert_allowed_subprocess(["codex", "exec"])
        with self.assertRaises(RuntimeError):
            advisor.assert_allowed_subprocess(["ollama", "run"])
        with self.assertRaises(RuntimeError):
            advisor.assert_allowed_subprocess(["bash", "-lc", "echo no"])

    def test_subprocess_allowlist_accepts_windows_wrapper_suffixes(self) -> None:
        advisor.assert_allowed_subprocess([r"C:\Tools\opencode.exe", "run"])
        advisor.assert_allowed_subprocess([r"C:\Tools\claude.cmd", "-p"])
        advisor.assert_allowed_subprocess([r"C:\Tools\agy.bat", "--print", ""])
        with self.assertRaises(RuntimeError):
            advisor.assert_allowed_subprocess([r"C:\Tools\codex.exe", "exec"])

    def test_windows_detach_kwargs_do_not_use_posix_sessions(self) -> None:
        with mock.patch.object(advisor, "IS_WINDOWS", True):
            kwargs = advisor.subprocess_detach_kwargs()
    ...
    def test_msvcrt_lock_fallback_is_used_without_fcntl(self) -> None:
        ...
        handle = FakeHandle()
        with mock.patch.object(advisor, "fcntl", None), mock.patch.object(advisor, "msvcrt", FakeMsvcrt):
            advisor.file_lock_exclusive(handle, blocking=False)
            advisor.file_unlock(handle)
    ...
    def test_doctor_marks_native_windows_shims_unverified_with_limited_snapshot(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            out = io.StringIO()
            original_exists = advisor.Path.exists
            with (
                mock.patch.object(advisor, "IS_WINDOWS", True),
                mock.patch.object(advisor.sys, "platform", "win32"),
                mock.patch.object(advisor.shutil, "which", return_value="/bin/tool"),
                mock.patch.object(advisor.platform, "system", return_value="Windows"),
                mock.patch.object(advisor.Path, "exists", lambda self: False if str(self) == "/proc" else original_exists(self)),
                contextlib.redirect_stdout(out),
            ):
                rc = advisor.doctor(cfg)
    ...
    def test_safe_session_id_matches_session_dir_validator(self) -> None:
        sid = advisor.safe_session_id()
        with tempfile.TemporaryDirectory() as td:
            self.assertEqual(advisor.session_dir(Path(td), sid), Path(td) / sid)
            with self.assertRaises(SystemExit):
                advisor.session_dir(Path(td), "../bad")
    ...
    def test_result_to_state_requires_provider_session_id(self) -> None:
        ...
        state = advisor.result_to_state(result)
...
class ConfigEditingAndGateTests(unittest.TestCase):
    def test_save_user_config_with_backup_and_set_model_shape(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            advisor.save_user_config_with_backup(path, {"version": advisor.DEFAULT_CONFIG["version"]})
            cfg = advisor.load_user_config_for_edit(path)
            cfg["models"] = {"sonnet": [{"kind": "claude", "model": "claude-sonnet-5", "provider": "claude", "effort": "low"}]}
            backup = advisor.save_user_config_with_backup(path, cfg)
            self.assertIsNotNone(backup)
            loaded = advisor.load_config(path)
    ...
    def test_config_backups_are_unique_and_atomic_private(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            path = Path(td) / "config.json"
            advisor.atomic_write_text(path, '{"version": 1}\n')
            first = advisor.save_user_config_with_backup(path, {"version": 1})
            second = advisor.save_user_config_with_backup(path, {"version": 1, "models": {"sonnet": [{"kind": "claude", "model": "claude-sonnet-5", "provider": "claude"}]}})
            self.assertNotEqual(first, second)
    ...
    def test_write_gate_accepts_only_strict_states(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            gate = advisor.write_gate(root, "unit", "pass", "ok")
            self.assertEqual(gate["state"], "pass")
            self.assertTrue((root / "gates" / "unit.json").exists())
            with self.assertRaises(SystemExit):
                advisor.write_gate(root, "bad", "deferred", "nope")

    def test_artifact_writes_are_private(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            artifact = advisor.make_artifact_dir(root, "review")
            runner = advisor.Runner(advisor.DEFAULT_CONFIG, artifact)
            raw = runner.write_raw("sonnet", "claude", "stdout", "stderr")
            advisor.atomic_write_text(artifact / "input.md", "secret")
    ...
    def test_auth_failure_becomes_needs_human_without_fallback(self) -> None:
        with tempfile.TemporaryDirectory() as td, mock.patch.object(advisor, "run_subprocess", return_value=(1, "", "unauthorized", 0.1)):
            runner = advisor.Runner(advisor.DEFAULT_CONFIG, Path(td))
            result = asyncio.run(runner.run_candidate(
                "sonnet",
            ...
            self.assertEqual(advisor.advisor_exit_code([result]), advisor.EXIT_NEEDS_HUMAN)
    ...
    def test_model_stdout_auth_words_do_not_force_needs_human(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td, mock.patch.object(advisor, "run_subprocess", return_value=(1, stdout, "", 0.1)):
            runner = advisor.Runner(advisor.DEFAULT_CONFIG, Path(td))
            result = asyncio.run(runner.run_candidate(
                "sonnet",
                {"kind": "claude", "model": "claude-sonnet-5", "provider": "claude"},
    ...
    def test_cli_error_stdout_auth_becomes_needs_human(self) -> None:
        with tempfile.TemporaryDirectory() as td, mock.patch.object(advisor, "run_subprocess", return_value=(1, "Error: IneligibleTierError", "", 0.1)):
            runner = advisor.Runner(advisor.DEFAULT_CONFIG, Path(td))
            result = asyncio.run(runner.run_candidate(
                "agy_image",
                {"kind": "agy", "model": "default", "provider": "agy", "command": "agy", "timeout_key": "agy"},
    ...
    def test_common_auth_messages_become_needs_human(self) -> None:
        ...
        for message in messages:
            with self.subTest(message=message), tempfile.TemporaryDirectory() as td, mock.patch.object(advisor, "run_subprocess", return_value=(1, "", message, 0.1)):
                runner = advisor.Runner(advisor.DEFAULT_CONFIG, Path(td))
                result = asyncio.run(runner.run_candidate(
                    "sonnet",
                    {"kind": "claude", "model": "claude-sonnet-5", "provider": "claude"},
    ...
    def test_exit_code_rejects_all_skipped_required_advisors(self) -> None:
        skipped = advisor.AdvisorResult(advisor="sonnet", status="skipped")
        ok = advisor.AdvisorResult(advisor="kimi", status="ok")
        self.assertEqual(advisor.advisor_exit_code([skipped], skipped_ok=True), advisor.EXIT_ERROR)
        self.assertEqual(advisor.advisor_exit_code([ok, skipped], skipped_ok=True), advisor.EXIT_OK)
...
class ConversationModeTests(unittest.TestCase):
    def args(self, **overrides):
        defaults = {
            "config": "/nonexistent/advisor-test-config.json",
            "mode": "vision",
            "advisors": None,
            "single_ok": False,
            "file": [],
    ...
    @staticmethod
    def ok_result(name: str = "agy_image") -> advisor.AdvisorResult:
        return advisor.AdvisorResult(
            advisor=name,
            status="ok",
            provider="agy",
            model="default",
    ...
    def test_start_and_ask_update_session_without_live_provider(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td, \
            mock.patch.object(advisor.Runner, "run_logical", fake_run_logical), \
            mock.patch.object(advisor.Runner, "run_locked", fake_run_locked):
            out = io.StringIO()
            start_args = self.args(artifact_root=td)
            with contextlib.redirect_stdout(out):
                self.assertEqual(asyncio.run(advisor.start_mode(start_args)), advisor.EXIT_OK)
            ...
            with contextlib.redirect_stdout(out):
                self.assertEqual(asyncio.run(advisor.ask_mode(ask_args)), advisor.EXIT_OK)
        ...
        self.assertEqual(session["advisors"]["agy_image"]["turns"], 2)
    ...
    def test_run_mode_rejects_images_for_non_vision_modes(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            with self.assertRaises(SystemExit):
                asyncio.run(advisor.run_mode(args))
    ...
    def test_run_mode_stages_image_paths_before_prompt_and_provider(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td, mock.patch.object(advisor.Runner, "run_logical", fake_run_logical):
            root = Path(td)
            ...
            with contextlib.redirect_stdout(out):
                self.assertEqual(asyncio.run(advisor.run_mode(args)), advisor.EXIT_OK)
    ...
    def test_multi_runs_multiple_turns_without_live_provider(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td, \
            ...
            with contextlib.redirect_stdout(out):
                self.assertEqual(asyncio.run(advisor.multi_mode(args)), advisor.EXIT_OK)
            meta = json.loads(out.getvalue())
    ...
    def test_job_mode_ignores_forward_compatible_result_fields(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            (job / "meta.json").write_text(json.dumps({
                "results": [{"advisor": "x", "status": "ok", "future_field": "ignored"}]
            }))
            out = io.StringIO()
            args = type("Args", (), {"job_ref": str(job), "artifact_root": td, "json": True})()
            with contextlib.redirect_stdout(out):
                rc = advisor.job_mode(args)
            payload = json.loads(out.getvalue())
        self.assertEqual(rc, advisor.EXIT_OK)
...
class SotaExplorerTests(unittest.TestCase):
    ...
    def fake_source_result(self, source, query, *, limit, since, wave, lane, timeout):
        return advisor.SotaSourceResult(
            source=source,
            evidence=[
                advisor.SotaEvidence(
                    id="",
                    source=source,
    ...
    def test_sota_query_plan_splits_two_waves(self) -> None:
        plan = advisor.sota_query_plan("video generation", 12)
    ...
    def test_sota_query_plan_cycles_beyond_template_count(self) -> None:
        plan = advisor.sota_query_plan("video generation", 14)
    ...
    def test_sources_for_lane_falls_back_to_selected_sources(self) -> None:
        self.assertEqual(advisor.sources_for_lane(["arxiv"], "applied"), ["arxiv"])

    def test_sota_profiles_are_validated_and_select_defaults(self) -> None:
        advisor.validate_config(advisor.DEFAULT_CONFIG)
        normal = advisor.sota_profile_config(advisor.DEFAULT_CONFIG["sota"], "normal")
        deep = advisor.sota_profile_config(advisor.DEFAULT_CONFIG["sota"], "deep")
        self.assertLess(normal["max_sources"], deep["max_sources"])
        self.assertIn("exa", normal["sources"])
        with self.assertRaises(SystemExit):
            advisor.sota_profile_config(advisor.DEFAULT_CONFIG["sota"], "unknown")

    def test_sota_default_profile_is_normal(self) -> None:
        with tempfile.TemporaryDirectory() as td, mock.patch.object(advisor, "fetch_sota_source", side_effect=self.fake_source_result):
            out = io.StringIO()
            ...
            with contextlib.redirect_stdout(out):
                rc = asyncio.run(advisor.sota_mode(args))
            meta = json.loads(out.getvalue())
        ...
        self.assertEqual(meta["sources"], advisor.DEFAULT_CONFIG["sota"]["profiles"]["normal"]["sources"])
    ...
    def test_sota_profile_normal_overrides_limits_and_sources(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td, mock.patch.object(advisor, "fetch_sota_source", side_effect=fake_source):
            out = io.StringIO()
    ...
    def test_compact_search_query_shortens_long_prompts(self) -> None:
        ...
        compact = advisor.compact_search_query(query, max_chars=140)
    ...
    def test_arxiv_query_variants_and_relevance_filter(self) -> None:
        variants = advisor.arxiv_query_variants("retrieval augmented generation evaluation")
        self.assertTrue(any('ti:"retrieval augmented generation"' in query for query, _ in variants))
        relevant, score = advisor.is_relevant_to_query(
        ...
        irrelevant, _ = advisor.is_relevant_to_query(
            "retrieval augmented generation evaluation",
    ...
    def test_semantic_requires_key_to_avoid_public_rate_limit(self) -> None:
        with mock.patch.dict(os.environ, {}, clear=True):
            enabled, reason = advisor.source_enabled("semantic")
    ...
    def test_fetch_tavily_uses_compact_query(self) -> None:
        ...
        with mock.patch.dict(os.environ, {"TAVILY_API_KEY": "x"}), mock.patch.object(advisor, "http_json", side_effect=fake_json):
            advisor.fetch_tavily(long_query, limit=2, since=None, wave=1, lane="applied", timeout=10)
    ...
    def test_fetch_tavily_filters_since_when_date_available(self) -> None:
        ...
        with mock.patch.dict(os.environ, {"TAVILY_API_KEY": "x"}), mock.patch.object(advisor, "http_json", side_effect=fake_json):
            result = advisor.fetch_tavily("small object detection", limit=5, since="2025-01-01", wave=1, lane="applied", timeout=10)
    ...
    def test_fetch_brave_does_not_treat_age_as_iso_date(self) -> None:
        ...
        with mock.patch.dict(os.environ, {"BRAVE_SEARCH_API_KEY": "x"}), mock.patch.object(advisor, "http_json", side_effect=fake_json):
            result = advisor.fetch_brave("small object detection", limit=5, since="2025-01-01", wave=1, lane="applied", timeout=10)
    ...
    def test_fetch_arxiv_filters_off_topic_atom_results(self) -> None:
        ...
        """
        with mock.patch.object(advisor, "http_text_retry", return_value=xml), \
            mock.patch.dict(os.environ, {"ADVISOR_SOTA_ARXIV_MIN_INTERVAL_SEC": "0"}):
            result = advisor.fetch_arxiv("retrieval augmented generation evaluation", limit=4, since="2025-01-01", wave=1, lane="academic", timeout=10)
        self.assertEqual(result.status, "ok")
    ...
    def test_cli_aliases_for_sota_profiles(self) -> None:
        with mock.patch.object(advisor, "sota_mode", return_value=advisor.EXIT_OK) as mocked:
            rc = advisor.cli_main(["@sota-normal", "topic", "--no-model"])
        self.assertEqual(rc, advisor.EXIT_OK)
        self.assertEqual(mocked.call_args.args[0].profile, "normal")
        with mock.patch.object(advisor, "sota_mode", return_value=advisor.EXIT_OK) as mocked:
            rc = advisor.cli_main(["@sota-deep", "topic", "--no-model"])
        self.assertEqual(rc, advisor.EXIT_OK)
        self.assertEqual(mocked.call_args.args[0].profile, "deep")
        with mock.patch.object(advisor, "sota_mode", return_value=advisor.EXIT_OK) as mocked:
            rc = advisor.cli_main(["sota", "topic", "--depth", "deep", "--no-model"])
        self.assertEqual(rc, advisor.EXIT_OK)
        self.assertEqual(mocked.call_args.args[0].profile, "deep")
    ...
    def test_sota_strict_topic_filters_off_topic_evidence(self) -> None:
        def fake_off_topic(source, query, *, limit, since, wave, lane, timeout):
            return advisor.SotaSourceResult(source, [
                advisor.SotaEvidence("", source, "https://example.com/off", "Calibrating galaxies", "paper", "2026-01-01", advisor.utc_now(), [], "Cosmology distance ladders.", query, wave, lane, "unit", 0.1, 0.5),
                advisor.SotaEvidence("", source, "https://example.com/on", "Agentic coding benchmark", "paper", "2026-01-01", advisor.utc_now(), [], "Agentic coding benchmarks and repository tasks.", query, wave, lane, "unit", 0.8, 0.7),
            ])
        with tempfile.TemporaryDirectory() as td, mock.patch.object(advisor, "fetch_sota_source", side_effect=fake_off_topic):
            out = io.StringIO()
    ...
    def test_classify_evidence_quality_labels_vendor_and_off_topic(self) -> None:
        vendor = advisor.SotaEvidence("", "exa", "https://www.pinecone.io/blog/rag-eval", "RAG evaluation benchmarks", "web", excerpt="Retrieval augmented generation evaluation and faithfulness.")
        spoofed = advisor.SotaEvidence("", "exa", "https://openai.com.evil.example/post", "RAG evaluation benchmarks", "web", excerpt="Retrieval augmented generation evaluation and faithfulness.")
        off = advisor.SotaEvidence("", "arxiv", "u", "Galaxy distance calibration", "paper", excerpt="Cosmology clustering measurements.")
        weak_paper = advisor.SotaEvidence("", "arxiv", "https://arxiv.org/abs/1", "Agentic systems", "paper", published_at="2026-01-01", excerpt="Agentic workflows.", relevance=0.9)
        self.assertEqual(advisor.classify_evidence_quality(vendor, "retrieval augmented generation evaluation")[0], "vendor")
        self.assertNotEqual(advisor.classify_evidence_quality(spoofed, "retrieval augmented generation evaluation")[0], "vendor")
        self.assertEqual(advisor.classify_evidence_quality(off, "retrieval augmented generation evaluation")[0], "off_topic")
        self.assertEqual(advisor.classify_evidence_quality(weak_paper, "agentic coding benchmarks")[0], "medium")
    ...
    def test_source_health_keeps_warnings_separate_from_errors(self) -> None:
        ...
        evidence = [advisor.SotaEvidence("E1", "brave", "https://example.com", "Agentic coding benchmark", "web")]
        health = advisor.build_source_health(events, evidence)
    ...
    def test_evidence_to_prompt_truncates_to_valid_json(self) -> None:
        evidence = [
            advisor.SotaEvidence(id=f"E{i}", source="arxiv", url=f"u{i}", title="title", source_type="paper", excerpt="x" * 500)
            for i in range(10)
        ]
        payload = advisor.evidence_to_prompt(evidence, max_chars=1200)
    ...
    def test_evidence_to_prompt_keeps_at_least_one_huge_item_when_possible(self) -> None:
        evidence = [advisor.SotaEvidence(id="E1", source="arxiv", url="u", title="title", source_type="paper", excerpt="x" * 100000)]
        payload = advisor.evidence_to_prompt(evidence, max_chars=2000)
        rows = json.loads(payload)
    ...
    def test_evidence_to_prompt_shortens_excerpts_before_dropping_items(self) -> None:
        evidence = [
            advisor.SotaEvidence(id=f"E{i}", source="arxiv", url=f"u{i}", title="title", source_type="paper", excerpt="x" * 1000)
            for i in range(3)
        ]
        payload = advisor.evidence_to_prompt(evidence, max_chars=2500)
        rows = json.loads(payload)
    ...
    def test_sota_reviewer_exception_falls_back_to_deterministic_report(self) -> None:
        ...
        async def fake_run_logical(self, name, prompt, files, images=None):
            ...
            return advisor.AdvisorResult(advisor=name, status="ok", content=f"{name} [E1]")

        with tempfile.TemporaryDirectory() as td, \
            mock.patch.object(advisor, "fetch_sota_source", side_effect=self.fake_source_result), \
            mock.patch.object(advisor.Runner, "run_logical", fake_run_logical):
            out = io.StringIO()
    ...
    def test_sota_no_model_without_evidence_is_insufficient(self) -> None:
        empty = advisor.SotaSourceResult("arxiv", [], "ok", None)
        with tempfile.TemporaryDirectory() as td, mock.patch.object(advisor, "fetch_sota_source", return_value=empty):
            out = io.StringIO()
            args = self.sota_args(artifact_root=td, no_model=True, source=["arxiv"], max_sources=2, max_queries=2)
            with contextlib.redirect_stdout(out):
                rc = asyncio.run(advisor.sota_mode(args))
    ...
    def test_sota_model_mode_without_evidence_skips_model_spend(self) -> None:
        ...
        async def fail_if_called(self, name, prompt, files, images=None):
            raise AssertionError("model advisor should not run without evidence")

        with tempfile.TemporaryDirectory() as td, \
            mock.patch.object(advisor, "fetch_sota_source", return_value=empty), \
            mock.patch.object(advisor.Runner, "run_logical", fail_if_called):
            out = io.StringIO()
            args = self.sota_args(artifact_root=td, no_model=False, source=["arxiv"], max_sources=2, max_queries=2)
            with contextlib.redirect_stdout(out):
                rc = asyncio.run(advisor.sota_mode(args))
    ...
    def test_sota_model_mode_requires_reviewer_before_spending_tokens(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            with self.assertRaises(SystemExit):
                asyncio.run(advisor.sota_mode(args))
    ...
    def test_fetch_sota_source_degrades_on_malformed_source_exception(self) -> None:
        for exc in (ValueError("bad date"), AttributeError("bad shape")):
            with self.subTest(exc=type(exc).__name__), \
                mock.patch.dict(advisor.SOTA_FETCHERS, {"arxiv": lambda *a, exc=exc, **k: (_ for _ in ()).throw(exc)}):
                result = advisor.fetch_sota_source("arxiv", "q", limit=1, since=None, wave=1, lane="academic", timeout=1)
            self.assertEqual(result.status, "error")
    ...
    def test_sota_high_uses_high_reviewer(self) -> None:
        ...
        self.assertEqual(meta["reviewer"]["advisor"], "fable")

    def test_sota_review_prompt_caps_synthesis_and_evidence_text(self) -> None:
        evidence = [
            advisor.SotaEvidence(id=f"E{i}", source="arxiv", url=f"u{i}", title="t", source_type="paper", excerpt="e" * 2000)
            for i in range(80)
        ]
        syntheses = [advisor.AdvisorResult(advisor="a", status="ok", content="x" * 100000)]
        prompt = advisor.build_sota_review_prompt("q", evidence, syntheses)
    ...
    def test_sota_model_pipeline_uses_two_synthesizers_and_reviewer(self) -> None:
        ...
        self.assertEqual(meta["reviewer"]["advisor"], "glm_max")

    def test_verify_sota_report_rejects_missing_evidence_ids(self) -> None:
        evidence = [advisor.SotaEvidence(id="E1", source="arxiv", url="u", title="t", source_type="paper")]
        result = advisor.verify_sota_report("Uses [E1] and [E99]", evidence)
        self.assertEqual(result["status"], "error")
    ...
    def test_verify_sota_report_rejects_unexpected_urls_and_invalid_ids(self) -> None:
        evidence = [advisor.SotaEvidence(id="bad", source="arxiv", url="https://allowed.example", title="t", source_type="paper")]
        result = advisor.verify_sota_report("Uses https://evil.example", evidence)
        self.assertEqual(result["status"], "error")
    ...
    def test_verify_sota_report_marks_empty_evidence_insufficient(self) -> None:
        result = advisor.verify_sota_report("Coverage insufficient.", [])
    ...
    def test_verify_sota_report_rejects_when_report_cites_no_ids(self) -> None:
        evidence = [advisor.SotaEvidence(id="E1", source="arxiv", url="u", title="t", source_type="paper")]
        result = advisor.verify_sota_report("Evidence exists but no bracketed IDs.", evidence)
        self.assertEqual(result["status"], "error")
    ...
    def test_verify_sota_report_normalizes_equivalent_urls(self) -> None:
        evidence = [advisor.SotaEvidence(id="E1", source="arxiv", url="https://www.example.com/path/", title="t", source_type="paper")]
        result = advisor.verify_sota_report("Uses [E1] http://example.com/path#section", evidence)
        self.assertEqual(result["status"], "ok")
...
class InternalBenchmarkTests(unittest.TestCase):
    def test_internal_benchmark_writes_versioned_artifacts(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            payload = advisor.run_internal_benchmark(advisor.DEFAULT_CONFIG, artifact, iterations=1)
            self.assertTrue((artifact / "benchmark.json").exists())
            self.assertTrue((artifact / "report.md").exists())
        self.assertEqual(payload["schema_version"], advisor.BENCHMARK_SCHEMA_VERSION)
        self.assertEqual(payload["suite_id"], advisor.BENCHMARK_SUITE_ID)
        self.assertEqual(payload["suite_version"], advisor.BENCHMARK_SUITE_VERSION)
    ...
    def test_internal_benchmark_comparison_reports_score_and_duration_delta(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            old_payload = advisor.run_internal_benchmark(advisor.DEFAULT_CONFIG, baseline, iterations=1)
            ...
            advisor.atomic_write_json(baseline / "benchmark.json", old_payload)
            payload = advisor.run_internal_benchmark(advisor.DEFAULT_CONFIG, current, iterations=1, compare_path=baseline)
    ...
    def test_cli_benchmark_alias_runs_without_provider_calls(self) -> None:
        with tempfile.TemporaryDirectory() as td, mock.patch.object(advisor, "DEFAULT_ARTIFACT_ROOT", Path(td)):
            out = io.StringIO()
            with contextlib.redirect_stdout(out):
                rc = advisor.cli_main(["bench", "--artifact-root", td, "--json"])
            payload = json.loads(out.getvalue())
        self.assertEqual(rc, advisor.EXIT_OK)
        self.assertEqual(payload["suite_id"], advisor.BENCHMARK_SUITE_ID)
        self.assertEqual(payload["case_count"], len(advisor.BENCHMARK_CASES))

    def test_prompt_variants_record_persona_metrics(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            no_persona = advisor.run_internal_benchmark(
                advisor.DEFAULT_CONFIG,
                Path(td) / "no-persona",
                iterations=1,
                prompt_variant="no-persona",
                benchmark_advisor="sonnet",
            )
            with_persona = advisor.run_internal_benchmark(
                advisor.DEFAULT_CONFIG,
                Path(td) / "persona",
                iterations=1,
    ...
    def test_compact_prompt_variant_applies_total_cap(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            payload = advisor.run_internal_benchmark(
                advisor.DEFAULT_CONFIG,
                Path(td) / "compact",
                iterations=1,
    ...
    def test_problem_suite_quality_scores_gold_above_weak_answers(self) -> None:
        suite = advisor.run_benchmark_problem_suite()
        self.assertEqual(suite["version"], advisor.BENCHMARK_PROBLEM_SET_VERSION)
    ...
    def test_internal_benchmark_includes_problem_suite_metrics(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            payload = advisor.run_internal_benchmark(advisor.DEFAULT_CONFIG, Path(td) / "bench", iterations=1)
        self.assertEqual(payload["problem_set_version"], advisor.BENCHMARK_PROBLEM_SET_VERSION)
    ...
    def test_problem_suite_separates_fixture_pass_from_headroom_and_hashes_scorer_params(self) -> None:
        ...
        self.assertEqual(suite["scorer_params"]["forbidden_cap"], advisor.BENCHMARK_FORBIDDEN_CAP)
        self.assertEqual(suite["scorer_params"]["negation_window_chars"], advisor.BENCHMARK_NEGATION_WINDOW_CHARS)

    def test_internal_benchmark_iterations_are_deterministic_and_reuse_problem_hashes(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            payload = advisor.run_internal_benchmark(advisor.DEFAULT_CONFIG, Path(td) / "bench", iterations=2)
    ...
    def test_forbidden_negation_guard_requires_all_occurrences_negated(self) -> None:
        ...
        negated = advisor.score_benchmark_problem_answer(problem, "safe: never silently fallback")
        mixed = advisor.score_benchmark_problem_answer(problem, "safe: never silently fallback, then silently fallback anyway")
        self.assertEqual(negated["forbidden_hits"], [])
        self.assertEqual(mixed["forbidden_hits"], ["silently fallback"])
        self.assertLessEqual(mixed["score"], advisor.BENCHMARK_FORBIDDEN_CAP)

    def test_fixture_hash_covers_prompt_and_control_answer(self) -> None:
        original = advisor.run_benchmark_problem_suite()["fixture_set_hash"]
        old_prompt = advisor.BENCHMARK_PROBLEMS[0]["prompt"]
        old_control = advisor.BENCHMARK_PROBLEMS[0]["control_answer"]
        try:
            advisor.BENCHMARK_PROBLEMS[0]["prompt"] = old_prompt + " changed"
            prompt_hash = advisor.run_benchmark_problem_suite()["fixture_set_hash"]
            advisor.BENCHMARK_PROBLEMS[0]["prompt"] = old_prompt
            advisor.BENCHMARK_PROBLEMS[0]["control_answer"] = "non-empty control"
            control_hash = advisor.run_benchmark_problem_suite()["fixture_set_hash"]
        finally:
            advisor.BENCHMARK_PROBLEMS[0]["prompt"] = old_prompt
            advisor.BENCHMARK_PROBLEMS[0]["control_answer"] = old_control
