#!/usr/bin/env python3
"""Benchmark v1 runner for mosaic-dev quality/SOTA/infra/cost axes.
...
MOSAIC_DEV = ROOT / "bin/mosaic-dev"
...
def score_quality(case: dict[str, Any], content: str, meta: dict[str, Any], duration: float, exit_code: int) -> dict[str, Any]:
    ...
    for result in meta.get("results", []):
        ...
        mosaics.append(result.get("mosaic"))
...
def run_quality_case(case: dict[str, Any], mosaic: str, artifact_root: Path, timeout: int) -> dict[str, Any]:
    ...
    cmd = [str(MOSAIC_DEV), mode, prompt, "--mosaic", mosaic, "--single-ok", "--file", str(ROOT / case["path"]), "--artifact-root", str(artifact_root), "--json"]
    exit_code, stdout, stderr, duration = run_cmd(cmd, timeout=timeout)
    if exit_code == 3:
        raise SystemExit("needs_human exit code from mosaic-dev; stopping benchmark immediately")
    try:
    ...
    if any((r.get("status") == "needs_human") for r in meta.get("results", [])):
        raise SystemExit("needs_human status from mosaic-dev; stopping benchmark immediately")
...
def run_infra_case(case: dict[str, Any], artifact_root: Path) -> dict[str, Any]:
    cmd = [str(MOSAIC_DEV), "benchmark", "--json", "--artifact-root", str(artifact_root)]
    exit_code, stdout, stderr, duration = run_cmd(cmd, timeout=120)
    if exit_code == 3:
        raise SystemExit("needs_human exit code from mosaic-dev internal benchmark; stopping benchmark immediately")
    try:
...
def render_report(payload: dict[str, Any]) -> str:
    lines = [
        f"# Mosaic benchmark v{payload['benchmark_version']} — {payload['profile']}",
        "",
        f"Generated: `{payload['generated_at']}`",
        f"Mosaic: `{payload['mosaic']}`",
...
def main() -> int:
    ...
    parser.add_argument("--mosaic", default="minimax")
    ...
    result_dir = ROOT / "benchmarks/results" / f"{utc_stamp()}-v{manifest['version']}-{args.profile}-{args.mosaic}"
    artifact_root = result_dir / "mosaic-artifacts"
    artifact_root.mkdir(parents=True, exist_ok=True)

    selected = [c for c in manifest["cases"] if args.profile == "full" or c.get("cheap")]
    quality_rows = [run_quality_case(case, args.mosaic, artifact_root, args.timeout) for case in selected]
    ...
    if args.json:
        ...
        print(f"mosaic-dev benchmark {args.profile}: score={total}/100 results={result_dir}")
    return 0
