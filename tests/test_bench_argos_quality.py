...
MODULE_PATH = Path(__file__).resolve().parents[1] / "scripts" / "bench_argos_quality.py"
SPEC = importlib.util.spec_from_file_location("bench_argos_quality", MODULE_PATH)
...
- (none)
## Minimal fix plan
1. Update `argos.py` to validate the status field.
...
def test_concrete_fix_target_rejects_generic_backticks_and_single_target() -> None:
    ...
    one_target = """1. Update `argos.py`.\n2. Run tests.\n"""
    two_targets = """1. Update `argos.py`.\n2. Add `test_status_validation`.\n"""
