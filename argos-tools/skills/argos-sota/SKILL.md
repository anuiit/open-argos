---
name: argos-sota
description: Run argos SOTA Explorer for source-backed deep web/academic research over a domain or question. Use `argos sota` / `argos @sota-explorer` with bounded sources, evidence artifacts, two synthesis models, and a final reviewer.
---
...
```bash
argos @sota-normal "<domain or question>" --json
argos @sota-deep "<domain or question>" --json
```

Use normal for a daily bounded pass and deep for a wider source/query budget. Plain `argos sota` defaults to normal.
