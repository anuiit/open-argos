#!/usr/bin/env python3
"""Non-destructive Argos-Tools/argos smoke test.
...
def main() -> int:
    ...
    doctor = run(["argos", "doctor"], check=False)
    doctor_data = parse_json_output(doctor, "argos doctor")
    core_ready = bool(doctor_data.get("readiness", {}).get("core_text_argoses"))
    ...
    if doctor.returncode != 0 and not ((args.vision or args.live) and agy_ready):
        raise SystemExit(f"argos doctor returned {doctor.returncode}; core text argos are not ready")
    ping = run(["argos", "ping", "--json"])
    data = parse_json_output(ping, "argos ping --json")
    if data.get("status") != "ok":
        raise SystemExit("argos ping returned non-ok status")
    run(["argos", "providers", "--json"])
    if not args.no_gate:
        gate_cmd = ["argos", "gate", "set", "argos-tools-smoke", "pass", "--evidence", "smoke script static checks passed"]
