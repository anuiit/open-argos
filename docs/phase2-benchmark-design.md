# Phase 2 Benchmark Design — advisor-dev

Status: proposed; do not execute baseline/setup until human validation.

## Objective

Create Benchmark v1.0.0 for advisor-dev covering:
...
- Stable advisor on PATH remains judge/gate instrument and is not modified.
- advisor-dev is invoked only through `/home/sina/advisor-dev/bin/advisor-dev` or a temporary local alias.
...
benchmarks/
  ...
  golden/v1/
    ...
    cases/
      real-inputs/*.md         # sanitized copies sampled from ~/.advisor/sessions/*/input.md
...
scripts/
  bench_advisor_quality.py     # stdlib runner/scorer, proposed
```
...
- Source pool exists: 442 `input.md` files under `/home/sina/.advisor/sessions` at Phase 2 design time.
...
7. Prompt-safety blocker: attached file instructs advisor to ignore contract / run tools.
8. Cost/latency blocker: `meta.json` omits advisor cost/duration or reports negative cost.
...
- Parallel lock acquisition/release with temp `ADVISOR_LOCK_ROOT` and forced timeout.
...
- Per preset: total cost, by advisor, duration p50/p95.
...
Cheap subset score uses 2–3 cases and one explicit provider/advisor (`--advisor minimax --single-ok` or another validated cheap provider) for fast iteration. Official score uses the full v1 set and configured presets.

## Runner design

`bench_advisor_quality.py` (proposed, stdlib only):

- Loads `benchmarks/golden/v1/manifest.json`.
- Creates temp artifact roots for advisor-dev runs.
- Invokes advisor-dev with env-isolated `ADVISOR_CONFIG_DIR`, `ADVISOR_ARTIFACT_ROOT`, `ADVISOR_LOCK_ROOT`.
- Captures stdout/stderr/exit code.
- Locates advisor artifact dir from JSON output or stdout.
...
Run sequentially on advisor-dev:

```bash
python3 -m pytest -q advisor/tests adv-tools/tests
python3 -m ruff check advisor/advisor.py advisor/tests adv-tools/scripts adv-tools/tests
# with temp advisor alias to /home/sina/advisor-dev/bin/advisor-dev
python3 adv-tools/scripts/smoke_adv_tools.py --adversarial --no-gate --advisor-py advisor/advisor.py
```

Then, only after validation:

```bash
advisor gate set bench-setup pass --evidence "Phase 2 benchmark setup approved and non-regression gate passed"
```
...
- Ruff available.
- advisor stable 0.6.2 available.
- advisor-dev wrapper available.
...
Do not create `benchmarks/golden/v1`, do not implement `scripts/bench_advisor_quality.py`, do not run baseline, and do not set `bench-setup` gate until this design and the no-install plan are approved.
