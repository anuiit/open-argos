# Phase 2 Benchmark Design — argos-dev

Status: proposed; do not execute baseline/setup until human validation.

## Objective

Create Benchmark v1.0.0 for argos-dev covering:
...
- Stable argos on PATH remains judge/gate instrument and is not modified.
- argos-dev is invoked only through `/home/sina/argos-dev/bin/argos-dev` or a temporary local alias.
...
benchmarks/
  ...
  golden/v1/
    ...
    cases/
      real-inputs/*.md         # sanitized copies sampled from ~/.argos/sessions/*/input.md
...
scripts/
  bench_argos_quality.py     # stdlib runner/scorer, proposed
```
...
- Source pool exists: 442 `input.md` files under `/home/sina/.argos/sessions` at Phase 2 design time.
...
7. Prompt-safety blocker: attached file instructs argos to ignore contract / run tools.
8. Cost/latency blocker: `meta.json` omits argos cost/duration or reports negative cost.
...
- Parallel lock acquisition/release with temp `ARGOS_LOCK_ROOT` and forced timeout.
...
- Per preset: total cost, by argos, duration p50/p95.
...
Cheap subset score uses 2–3 cases and one explicit provider/argos (`--argos minimax --single-ok` or another validated cheap provider) for fast iteration. Official score uses the full v1 set and configured presets.

## Runner design

`bench_argos_quality.py` (proposed, stdlib only):

- Loads `benchmarks/golden/v1/manifest.json`.
- Creates temp artifact roots for argos-dev runs.
- Invokes argos-dev with env-isolated `ARGOS_CONFIG_DIR`, `ARGOS_ARTIFACT_ROOT`, `ARGOS_LOCK_ROOT`.
- Captures stdout/stderr/exit code.
- Locates argos artifact dir from JSON output or stdout.
...
Run sequentially on argos-dev:

```bash
python3 -m pytest -q argos/tests argos-tools/tests
python3 -m ruff check argos/argos.py argos/tests argos-tools/scripts argos-tools/tests
# with temp argos alias to /home/sina/argos-dev/bin/argos-dev
python3 argos-tools/scripts/smoke_argos_tools.py --adversarial --no-gate --argos-py argos/argos.py
```

Then, only after validation:

```bash
argos gate set bench-setup pass --evidence "Phase 2 benchmark setup approved and non-regression gate passed"
```
...
- Ruff available.
- argos stable 0.6.2 available.
- argos-dev wrapper available.
...
Do not create `benchmarks/golden/v1`, do not implement `scripts/bench_argos_quality.py`, do not run baseline, and do not set `bench-setup` gate until this design and the no-install plan are approved.
