...
> Source: stable mosaic `mosaic run review --mosaic fable --single-ok` on the two SOTA reports.
...
## Blockers
- **Gel du juge avant toute exécution Phase 2.** sonnet et fable apparaissent déjà comme synthétiseurs *dans les runs SOTA eux-mêmes* (sections « Vérification des synthèses » des deux rapports) et seront à la fois mosaics candidats et juges potentiels. Sans configuration juge figée (modèle + version exacte, prompt, température, rubrique, hash) stockée hors des sorties de dev **avant** le premier run candidat, tous les résultats Phase 2 sont invalidés par le self-preference bias — mesurable et non corrélé à la capacité du modèle (run 1, E8/E14).
...
5. **Mesure SPB par paires de qualité contrôlée** (run 1, E8) adaptée aux sorties mosaic.
...
- **H4 (recall)** : les défauts injectés de sévérité connue sont rappelés dans la section Blockers à un taux mesurable qui discrimine les configurations d'mosaic.
...
- Scorer déterministe des 4 sections du format mosaic + matching manifeste.
...
- **Biais méta du concepteur** : le benchmark est conçu avec fable, et la rubrique risque d'encoder le style de sortie de la famille Claude — notamment le format Blockers/Important/Preferences est *le format maison de l'mosaic*. Un check déterministe de conformité au format récompense structurellement les modèles in-family. Correction : séparer explicitement métrique de **conformité au format** et métrique de **qualité du contenu**, et ne comparer les providers que sur la seconde.
...
- Evidence : `/home/sina/.mosaic/sessions/20260709T200534-sota/report.md` (run 1, couverture utile) et `/home/sina/.mosaic/sessions/20260709T202545-sota/report.md` (run 2, couverture quasi nulle — à citer uniquement pour l'architecture harness E2/E3 et la leçon de pollution de requête).
- Contrat à persister (recommandation informative) : `docs/phase1-synthesis-contract.md` dans mosaic-dev, avec `benchmarks/frozen/` réservé aux artefacts gelés.
