...
spec = importlib.util.spec_from_file_location("argos_under_test", ARGOS_PATH)
...
class ConfigValidationTests(unittest.TestCase):
    ...
    def test_load_env_file_does_not_override_existing_values(self) -> None:
        with tempfile.TemporaryDirectory() as td, mock.patch.dict(os.environ, {"ARGOS_TEST_ENV": "original"}, clear=False):
            ...
            env_path.write_text("ARGOS_TEST_ENV=changed\nARGOS_TEST_NEW=loaded\n# ignored\nBAD-KEY=nope\n", encoding="utf-8")
            try:
                os.environ.pop("ARGOS_TEST_NEW", None)
                argos.load_env_file(env_path)
                self.assertEqual(os.environ["ARGOS_TEST_ENV"], "original")
                self.assertEqual(os.environ["ARGOS_TEST_NEW"], "loaded")
    ...
    def test_preset_cross_references_are_validated(self) -> None:
        cfg = argos.deep_merge(argos.DEFAULT_CONFIG, {
            "presets": {"@bad": {"mode": "review", "argoses": ["missing"]}},
        })
        with self.assertRaises(SystemExit):
            argos.validate_config(cfg)
    ...
    def test_validate_config_does_not_brick_core_for_sota_model_refs(self) -> None:
        cfg = argos.deep_merge(argos.DEFAULT_CONFIG, {"sota": {"reviewer": "missing", "synthesizers": ["also_missing"]}})
        argos.validate_config(cfg)


    def test_agy_default_concurrency_is_conservative(self) -> None:
        self.assertEqual(argos.concurrency_limit(argos.DEFAULT_CONFIG, "agy"), 2)

    def test_default_config_has_only_agy_vision_provider(self) -> None:
        self.assertNotIn("ge" + "mini_image", argos.DEFAULT_CONFIG["models"])
        self.assertNotIn("ge" + "mini", argos.DEFAULT_CONFIG["concurrency"])
        self.assertEqual(argos.DEFAULT_CONFIG["modes"]["vision"], ["agy_image"])
    ...
    def test_cli_main_maps_invalid_config_to_exit_error(self) -> None:
        with tempfile.TemporaryDirectory() as td, contextlib.redirect_stderr(io.StringIO()) as err:
            ...
            rc = argos.cli_main(["--config", str(bad), "doctor"])
    ...
    def test_validate_config_rejects_malformed_candidates(self) -> None:
        ...
        for override in bad_configs:
            with self.subTest(override=override):
                ...
                cfg = argos.deep_merge(argos.DEFAULT_CONFIG, test_override)
                with self.assertRaises(SystemExit):
                    argos.validate_config(cfg)
    ...
    def test_validate_config_rejects_invalid_runtime_numbers(self) -> None:
        ...
        for override in bad_overrides:
            with self.subTest(override=override):
                cfg = argos.deep_merge(argos.DEFAULT_CONFIG, override)
                with self.assertRaises(SystemExit):
                    argos.validate_config(cfg)
...
class PromptAndRoutingTests(unittest.TestCase):
    def test_build_prompt_embeds_file_once_with_cap(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            cfg = argos.deep_merge(argos.DEFAULT_CONFIG, {"limits": {"file_chars": 3}})
            prompt = argos.build_prompt("review", "please review", [path], cfg)
    ...
    def test_build_prompt_applies_total_prompt_cap(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            cfg = argos.deep_merge(argos.DEFAULT_CONFIG, {"limits": {"file_chars": 100, "total_prompt_chars": 120}})
            prompt = argos.build_prompt("review", "please review", [p1, p2], cfg)
    ...
    def test_build_prompt_total_cap_is_strict_for_tiny_limits(self) -> None:
        cfg = argos.deep_merge(argos.DEFAULT_CONFIG, {"limits": {"total_prompt_chars": 10}})
        prompt = argos.build_prompt("review", "x" * 100, [], cfg)
        self.assertLessEqual(len(prompt), 10)

    def test_build_prompt_lists_image_paths_and_mime(self) -> None:
        image = Path(tempfile.gettempdir()) / "argos-test-image.png"
        prompt = argos.build_prompt("vision", "describe", [], argos.DEFAULT_CONFIG, [image])
    ...
    def test_build_prompt_marks_embedded_files_as_untrusted(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            prompt = argos.build_prompt("review", "please review", [path], argos.DEFAULT_CONFIG)
    ...
    def test_build_prompt_includes_structured_output_contract(self) -> None:
        prompt = argos.build_prompt("review", "please review", [], argos.DEFAULT_CONFIG)
    ...
    def test_truncate_prompt_total_disabled_with_non_positive_limit(self) -> None:
        ...
        self.assertEqual(argos.truncate_prompt_total("abc", cfg), "abc")

    def test_resolve_preset_uses_preset_argoses_unless_explicit(self) -> None:
        mode, argoses, preset = argos.resolve_mode_and_argoses("@review", None, argos.DEFAULT_CONFIG)
        self.assertEqual(mode, "review")
        self.assertEqual(argoses, ["sonnet", "kimi", "minimax"])
        self.assertEqual(preset, "@review")

        mode, argoses, preset = argos.resolve_mode_and_argoses("@review", ["sonnet"], argos.DEFAULT_CONFIG)
        self.assertEqual(mode, "review")
        self.assertEqual(argoses, ["sonnet"])
        self.assertEqual(preset, "@review")

    def test_resolve_raw_mode_uses_configured_default_argoses(self) -> None:
        mode, argoses, preset = argos.resolve_mode_and_argoses("review", None, argos.DEFAULT_CONFIG)
        self.assertEqual(mode, "review")
        self.assertEqual(argoses, ["sonnet", "kimi", "minimax"])
        self.assertIsNone(preset)

    def test_resolve_raw_mode_keeps_explicit_argoses(self) -> None:
        mode, argoses, preset = argos.resolve_mode_and_argoses("review", ["sonnet"], argos.DEFAULT_CONFIG)
        self.assertEqual(mode, "review")
        self.assertEqual(argoses, ["sonnet"])
        self.assertIsNone(preset)
...
class ProviderParsingTests(unittest.TestCase):
    def test_parse_opencode_jsonl_text_and_usage(self) -> None:
        ...
        content, meta = argos.parse_opencode(stdout)
    ...
    def test_parse_claude_json(self) -> None:
        content, meta = argos.parse_claude(json.dumps({
            "result": " done ",
            "session_id": "claude_1",
            "total_cost_usd": 0.34,
    ...
    def test_parse_claude_ignores_surrounding_cli_noise(self) -> None:
        content, meta = argos.parse_claude('warning before\n' + json.dumps({
            "result": " ok ",
            "session_id": "claude_2",
    ...
    def test_parse_agy_text(self) -> None:
        content, meta = argos.parse_agy("visual answer\n")
...
class ArtifactTests(unittest.TestCase):
    def test_make_artifact_dir_avoids_same_second_collision_and_updates_latest(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td, mock.patch.object(argos.dt, "datetime", FixedDateTime):
            root = Path(td)
            first = argos.make_artifact_dir(root, "review")
            second = argos.make_artifact_dir(root, "review")
...
class SubprocessTimeoutTests(unittest.TestCase):
    ...
    def test_stage_vision_images_is_idempotent_for_already_staged_inputs(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            first = argos.stage_vision_images(root, [original])
            second = argos.stage_vision_images(root, first)
...
class RunListingTests(unittest.TestCase):
    def test_list_runs_reports_one_shot_artifacts_without_sessions(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            (run / "meta.json").write_text(json.dumps({
                "mode": "review",
                "results": [
                    {"argos": "sonnet", "status": "ok"},
                    {"argos": "kimi", "status": "error"},
                ],
            }), encoding="utf-8")
            ...
            with contextlib.redirect_stdout(out):
                rc = argos.list_runs(root, as_json=True)
    ...
    def test_list_sessions_still_reports_only_multiturn_sessions(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            out = io.StringIO()
            with contextlib.redirect_stdout(out):
                rc = argos.list_sessions(root, as_json=True)
            rows = json.loads(out.getvalue())
...
class SafetyAndSessionTests(unittest.TestCase):
    def test_subprocess_allowlist_blocks_codex_and_ollama(self) -> None:
        ...
        argos.assert_allowed_subprocess(["agy", "--print", "x"])
        with self.assertRaises(RuntimeError):
            argos.assert_allowed_subprocess(["ge" + "mini", "--prompt", "x"])
        with self.assertRaises(RuntimeError):
            argos.assert_allowed_subprocess(["codex", "exec"])
        with self.assertRaises(RuntimeError):
            argos.assert_allowed_subprocess(["ollama", "run"])
        with self.assertRaises(RuntimeError):
            argos.assert_allowed_subprocess(["bash", "-lc", "echo no"])

    def test_subprocess_allowlist_accepts_windows_wrapper_suffixes(self) -> None:
        argos.assert_allowed_subprocess([r"C:\Tools\opencode.exe", "run"])
        argos.assert_allowed_subprocess([r"C:\Tools\claude.cmd", "-p"])
        argos.assert_allowed_subprocess([r"C:\Tools\agy.bat", "--print", ""])
        with self.assertRaises(RuntimeError):
            argos.assert_allowed_subprocess([r"C:\Tools\codex.exe", "exec"])

    def test_windows_detach_kwargs_do_not_use_posix_sessions(self) -> None:
        with mock.patch.object(argos, "IS_WINDOWS", True):
            kwargs = argos.subprocess_detach_kwargs()
        self.assertIn("creationflags", kwargs)
        self.assertNotIn("start_new_session", kwargs)

    def test_sigkill_constant_is_windows_safe(self) -> None:
        self.assertEqual(argos.SIGKILL, getattr(signal, "SIGKILL", signal.SIGTERM))
        for func in (argos.terminate_process_group, argos.run_subprocess):
            self.assertNotIn("signal.SIGKILL", inspect.getsource(func))

    def test_terminate_process_group_windows_uses_taskkill(self) -> None:
        proc = mock.Mock(pid=1234)
        run = mock.Mock(return_value=mock.Mock(returncode=0))
        with mock.patch.object(argos, "IS_WINDOWS", True), mock.patch.object(argos.subprocess, "run", run):
            argos.terminate_process_group(proc, argos.SIGKILL)
    ...
    def test_windows_kill_tree_tolerates_nonzero_taskkill_rc(self) -> None:
        proc = mock.Mock(pid=4321)
        run = mock.Mock(return_value=mock.Mock(returncode=128))
    ...
    def test_windows_kill_tree_falls_back_to_proc_kill_without_taskkill(self) -> None:
        proc = mock.Mock(pid=4321)
        with mock.patch.object(argos.subprocess, "run", side_effect=FileNotFoundError("taskkill")):
            argos._windows_kill_tree(proc)
        proc.kill.assert_called_once()

    def test_posix_terminate_process_group_paths_are_unchanged(self) -> None:
        if not hasattr(signal, "SIGKILL"):
            self.skipTest("POSIX-only semantics: SIGKILL collapses to SIGTERM on this platform")

        proc = mock.Mock(pid=555)
        killpg = mock.Mock()
        with mock.patch.object(argos, "IS_WINDOWS", False), mock.patch.object(argos.os, "killpg", killpg):
            argos.terminate_process_group(proc, argos.SIGKILL)
        killpg.assert_called_once_with(555, argos.SIGKILL)
        proc.kill.assert_called_once()
        proc.terminate.assert_not_called()

        proc = mock.Mock(pid=556)
        killpg = mock.Mock()
        with mock.patch.object(argos, "IS_WINDOWS", False), mock.patch.object(argos.os, "killpg", killpg):
            argos.terminate_process_group(proc, signal.SIGTERM)
    ...
    def test_msvcrt_lock_fallback_is_used_without_fcntl(self) -> None:
        ...
        handle = FakeHandle()
        with mock.patch.object(argos, "fcntl", None), mock.patch.object(argos, "msvcrt", FakeMsvcrt):
            argos.file_lock_exclusive(handle, blocking=False)
            argos.file_unlock(handle)
    ...
    def test_safe_session_id_matches_session_dir_validator(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td:
            self.assertEqual(argos.session_dir(Path(td), sid), Path(td) / sid)
            with self.assertRaises(SystemExit):
                argos.session_dir(Path(td), "../bad")

    def test_result_to_state_requires_provider_session_id(self) -> None:
        result = argos.ArgosResult(
            argos="sonnet",
            status="ok",
            kind="claude",
            provider="claude",
        ...
        state = argos.result_to_state(result)
...
class ConfigEditingAndGateTests(unittest.TestCase):
    def test_save_user_config_with_backup_and_set_model_shape(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            argos.save_user_config_with_backup(path, {"version": argos.DEFAULT_CONFIG["version"]})
            cfg = argos.load_user_config_for_edit(path)
            cfg["models"] = {"sonnet": [{"kind": "claude", "model": "claude-sonnet-5", "provider": "claude", "effort": "low"}]}
            backup = argos.save_user_config_with_backup(path, cfg)
    ...
    def test_config_backups_are_unique_and_atomic_private(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            path = Path(td) / "config.json"
            argos.atomic_write_text(path, '{"version": 1}\n')
            first = argos.save_user_config_with_backup(path, {"version": 1})
            second = argos.save_user_config_with_backup(path, {"version": 1, "models": {"sonnet": [{"kind": "claude", "model": "claude-sonnet-5", "provider": "claude"}]}})
            self.assertNotEqual(first, second)
    ...
    def test_write_gate_accepts_only_strict_states(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            gate = argos.write_gate(root, "unit", "pass", "ok")
            self.assertEqual(gate["state"], "pass")
            self.assertTrue((root / "gates" / "unit.json").exists())
            with self.assertRaises(SystemExit):
                argos.write_gate(root, "bad", "deferred", "nope")
    ...
    def test_cli_error_stdout_auth_becomes_needs_human(self) -> None:
        with tempfile.TemporaryDirectory() as td, mock.patch.object(argos, "run_subprocess", return_value=(1, "Error: IneligibleTierError", "", 0.1)):
            runner = argos.Runner(argos.DEFAULT_CONFIG, Path(td))
            result = asyncio.run(runner.run_candidate(
                "agy_image",
                {"kind": "agy", "model": "default", "provider": "agy", "command": "agy", "timeout_key": "agy"},
    ...
    def test_common_auth_messages_become_needs_human(self) -> None:
        ...
        for message in messages:
            with self.subTest(message=message), tempfile.TemporaryDirectory() as td, mock.patch.object(argos, "run_subprocess", return_value=(1, "", message, 0.1)):
                runner = argos.Runner(argos.DEFAULT_CONFIG, Path(td))
                result = asyncio.run(runner.run_candidate(
                    "sonnet",
                    {"kind": "claude", "model": "claude-sonnet-5", "provider": "claude"},
    ...
    def test_exit_code_rejects_all_skipped_required_argoses(self) -> None:
        skipped = argos.ArgosResult(argos="sonnet", status="skipped")
        ok = argos.ArgosResult(argos="kimi", status="ok")
        self.assertEqual(argos.argos_exit_code([skipped], skipped_ok=True), argos.EXIT_ERROR)
        self.assertEqual(argos.argos_exit_code([ok, skipped], skipped_ok=True), argos.EXIT_OK)
...
class ConversationModeTests(unittest.TestCase):
    ...
    @staticmethod
    def ok_result(name: str = "agy_image") -> argos.ArgosResult:
    ...
    def test_run_mode_rejects_images_for_non_vision_modes(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            with self.assertRaises(SystemExit):
                asyncio.run(argos.run_mode(args))
    ...
    def test_multi_runs_multiple_turns_without_live_provider(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td, \
            ...
            with contextlib.redirect_stdout(out):
                self.assertEqual(asyncio.run(argos.multi_mode(args)), argos.EXIT_OK)
            meta = json.loads(out.getvalue())
    ...
    def test_job_mode_ignores_forward_compatible_result_fields(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            (job / "meta.json").write_text(json.dumps({
                "results": [{"argos": "x", "status": "ok", "future_field": "ignored"}]
            }))
            out = io.StringIO()
...
class SotaExplorerTests(unittest.TestCase):
    ...
    def test_sota_query_plan_splits_two_waves(self) -> None:
        plan = argos.sota_query_plan("video generation", 12)
    ...
    def test_sota_query_plan_cycles_beyond_template_count(self) -> None:
        plan = argos.sota_query_plan("video generation", 14)
    ...
    def test_sota_profiles_are_validated_and_select_defaults(self) -> None:
        ...
        normal = argos.sota_profile_config(argos.DEFAULT_CONFIG["sota"], "normal")
    ...
    def test_sota_default_profile_is_normal(self) -> None:
        ...
        self.assertEqual(meta["sources"], argos.DEFAULT_CONFIG["sota"]["profiles"]["normal"]["sources"])
    ...
    def test_sota_profile_normal_overrides_limits_and_sources(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td, mock.patch.object(argos, "fetch_sota_source", side_effect=fake_source):
            out = io.StringIO()
    ...
    def test_compact_search_query_shortens_long_prompts(self) -> None:
        ...
        compact = argos.compact_search_query(query, max_chars=140)
    ...
    def test_arxiv_query_variants_and_relevance_filter(self) -> None:
        ...
        irrelevant, _ = argos.is_relevant_to_query(
            "retrieval augmented generation evaluation",
    ...
    def test_fetch_tavily_uses_compact_query(self) -> None:
        ...
        with mock.patch.dict(os.environ, {"TAVILY_API_KEY": "x"}), mock.patch.object(argos, "http_json", side_effect=fake_json):
            argos.fetch_tavily(long_query, limit=2, since=None, wave=1, lane="applied", timeout=10)
    ...
    def test_fetch_tavily_filters_since_when_date_available(self) -> None:
        ...
        with mock.patch.dict(os.environ, {"TAVILY_API_KEY": "x"}), mock.patch.object(argos, "http_json", side_effect=fake_json):
            result = argos.fetch_tavily("small object detection", limit=5, since="2025-01-01", wave=1, lane="applied", timeout=10)
    ...
    def test_fetch_brave_does_not_treat_age_as_iso_date(self) -> None:
        ...
        with mock.patch.dict(os.environ, {"BRAVE_SEARCH_API_KEY": "x"}), mock.patch.object(argos, "http_json", side_effect=fake_json):
    ...
    def test_source_health_keeps_warnings_separate_from_errors(self) -> None:
        ...
        evidence = [argos.SotaEvidence("E1", "brave", "https://example.com", "Agentic coding benchmark", "web")]
        health = argos.build_source_health(events, evidence)
    ...
    def test_sota_reviewer_exception_falls_back_to_deterministic_report(self) -> None:
        ...
        async def fake_run_logical(self, name, prompt, files, images=None):
            ...
            return argos.ArgosResult(argos=name, status="ok", content=f"{name} [E1]")
    ...
    def test_sota_model_mode_requires_reviewer_before_spending_tokens(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            with self.assertRaises(SystemExit):
                asyncio.run(argos.sota_mode(args))
    ...
    def test_sota_review_prompt_caps_synthesis_and_evidence_text(self) -> None:
        ...
        prompt = argos.build_sota_review_prompt("q", evidence, syntheses)
    ...
    def test_verify_sota_report_marks_empty_evidence_insufficient(self) -> None:
        result = argos.verify_sota_report("Coverage insufficient.", [])
...
class InternalBenchmarkTests(unittest.TestCase):
    def test_internal_benchmark_writes_versioned_artifacts(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            payload = argos.run_internal_benchmark(argos.DEFAULT_CONFIG, artifact, iterations=1)
            self.assertTrue((artifact / "benchmark.json").exists())
            self.assertTrue((artifact / "report.md").exists())
        self.assertEqual(payload["schema_version"], argos.BENCHMARK_SCHEMA_VERSION)
        self.assertEqual(payload["suite_id"], argos.BENCHMARK_SUITE_ID)
    ...
    def test_internal_benchmark_comparison_reports_score_and_duration_delta(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            old_payload = argos.run_internal_benchmark(argos.DEFAULT_CONFIG, baseline, iterations=1)
            ...
            argos.atomic_write_json(baseline / "benchmark.json", old_payload)
            payload = argos.run_internal_benchmark(argos.DEFAULT_CONFIG, current, iterations=1, compare_path=baseline)
    ...
    def test_cli_benchmark_alias_runs_without_provider_calls(self) -> None:
        with tempfile.TemporaryDirectory() as td, mock.patch.object(argos, "DEFAULT_ARTIFACT_ROOT", Path(td)):
            out = io.StringIO()
            with contextlib.redirect_stdout(out):
                rc = argos.cli_main(["bench", "--artifact-root", td, "--json"])
            payload = json.loads(out.getvalue())
        self.assertEqual(rc, argos.EXIT_OK)
        self.assertEqual(payload["suite_id"], argos.BENCHMARK_SUITE_ID)
        self.assertEqual(payload["case_count"], len(argos.BENCHMARK_CASES))

    def test_prompt_variants_record_persona_metrics(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            no_persona = argos.run_internal_benchmark(
                argos.DEFAULT_CONFIG,
                Path(td) / "no-persona",
                iterations=1,
                prompt_variant="no-persona",
                benchmark_argos="sonnet",
            )
            with_persona = argos.run_internal_benchmark(
                argos.DEFAULT_CONFIG,
                Path(td) / "persona",
                iterations=1,
    ...
    def test_compact_prompt_variant_applies_total_cap(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            payload = argos.run_internal_benchmark(
                argos.DEFAULT_CONFIG,
                Path(td) / "compact",
                iterations=1,
    ...
    def test_internal_benchmark_includes_problem_suite_metrics(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            payload = argos.run_internal_benchmark(argos.DEFAULT_CONFIG, Path(td) / "bench", iterations=1)
        self.assertEqual(payload["problem_set_version"], argos.BENCHMARK_PROBLEM_SET_VERSION)
    ...
    def test_problem_suite_separates_fixture_pass_from_headroom_and_hashes_scorer_params(self) -> None:
        ...
        self.assertEqual(suite["scorer_params"]["forbidden_cap"], argos.BENCHMARK_FORBIDDEN_CAP)
        self.assertEqual(suite["scorer_params"]["negation_window_chars"], argos.BENCHMARK_NEGATION_WINDOW_CHARS)

    def test_internal_benchmark_iterations_are_deterministic_and_reuse_problem_hashes(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            payload = argos.run_internal_benchmark(argos.DEFAULT_CONFIG, Path(td) / "bench", iterations=2)
    ...
    def test_forbidden_negation_guard_requires_all_occurrences_negated(self) -> None:
        ...
        negated = argos.score_benchmark_problem_answer(problem, "safe: never silently fallback")
        mixed = argos.score_benchmark_problem_answer(problem, "safe: never silently fallback, then silently fallback anyway")
        self.assertEqual(negated["forbidden_hits"], [])
        self.assertEqual(mixed["forbidden_hits"], ["silently fallback"])
        self.assertLessEqual(mixed["score"], argos.BENCHMARK_FORBIDDEN_CAP)

    def test_fixture_hash_covers_prompt_and_control_answer(self) -> None:
        original = argos.run_benchmark_problem_suite()["fixture_set_hash"]
        old_prompt = argos.BENCHMARK_PROBLEMS[0]["prompt"]
        old_control = argos.BENCHMARK_PROBLEMS[0]["control_answer"]
        try:
            argos.BENCHMARK_PROBLEMS[0]["prompt"] = old_prompt + " changed"
            prompt_hash = argos.run_benchmark_problem_suite()["fixture_set_hash"]
            argos.BENCHMARK_PROBLEMS[0]["prompt"] = old_prompt
            argos.BENCHMARK_PROBLEMS[0]["control_answer"] = "non-empty control"
            control_hash = argos.run_benchmark_problem_suite()["fixture_set_hash"]
        finally:
            argos.BENCHMARK_PROBLEMS[0]["prompt"] = old_prompt
